...
## Goals
- Talk to Fortclaw via Discord DMs or guild channels.
...
3. Configure Fortclaw with `channels.discord.token` (or `DISCORD_BOT_TOKEN` as a fallback).
...
This is the “Discord Developer Portal” setup for running Fortclaw in a server (guild) channel like `#help`.
...
### 2) Enable the gateway intents Fortclaw needs
...
Discord uses numeric ids everywhere; Fortclaw config prefers ids.
...
- On startup, Fortclaw resolves channel/user names in allowlists to IDs (when the bot can search members)
  and logs the mapping; unresolved entries are kept as typed.

Native command notes:
- The registered commands mirror Fortclaw’s chat commands.
...
- Slash commands may still be visible in Discord UI to users who aren’t allowlisted; Fortclaw enforces allowlists on execution and replies “not authorized”.
