...
3) Set tokens for Fortclaw and start the gateway.
...
### User token (optional)
Fortclaw can use a Slack user token (`xoxp-...`) for read operations (history,
...
- Write operations (send/edit/delete messages, add/remove reactions, pin/unpin,
  ...
  no bot token is available, Fortclaw falls back to the user token.
...
```json
{
  "display_information": {
    "name": "Fortclaw",
    "description": "Slack connector for Fortclaw"
  },
  "features": {
    "bot_user": {
      "display_name": "Fortclaw",
      "always_online": false
    },
    "app_home": {
    ...
    "slash_commands": [
      {
        "command": "/clawd",
        "description": "Send a message to Fortclaw",
        "should_escape": false
      }
    ]
  },
...
## Reply threading
By default, Fortclaw replies in the main channel. Use `channels.slack.replyToMode` to control automatic threading:
...
- If Slack doesn’t provide `channel_type`, Fortclaw infers it from the channel ID prefix (`D`, `C`, `G`) and defaults to `channel` to keep session keys stable.
...
- `allowlist` requires channels to be listed in `channels.slack.channels`.
 ...
 - On startup, Fortclaw resolves channel/user names in allowlists to IDs (when tokens allow)
