...
Fortclaw exposes OAuth expiry health via `moltbot models status`. Use that for
...
- `scripts/claude-auth-status.sh`: Claude Code + Fortclaw auth checker (full/json/simple).
...
- `scripts/termux-sync-widget.sh`: sync Claude Code creds → Fortclaw.
