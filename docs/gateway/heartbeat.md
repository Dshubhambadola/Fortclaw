...
- During heartbeat runs, Fortclaw treats `HEARTBEAT_OK` as an ack when it appears
...
If **all three** are false, Fortclaw skips the heartbeat run entirely (no model call).
