...
- [ ] Update CLI/version strings: [`src/cli/program.ts`](https://github.com/Dshubhambadola/Fortclaw/blob/main/src/cli/program.ts) and the Baileys user agent in [`src/provider-web.ts`](https://github.com/Dshubhambadola/Fortclaw/blob/main/src/provider-web.ts).
- [ ] Confirm package metadata (name, description, repository, keywords, license) and `bin` map points to [`fortclaw.mjs`](https://github.com/Dshubhambadola/Fortclaw/blob/main/fortclaw.mjs) for `moltbot`.
- [ ] If dependencies changed, run `pnpm install` so `pnpm-lock.yaml` is current.

2) **Build & artifacts**
- [ ] If A2UI inputs changed, run `pnpm canvas:a2ui:bundle` and commit any updated [`src/canvas-host/a2ui/a2ui.bundle.js`](https://github.com/Dshubhambadola/Fortclaw/blob/main/src/canvas-host/a2ui/a2ui.bundle.js).
...
- [ ] Generate the Sparkle appcast (HTML notes via [`scripts/make_appcast.sh`](https://github.com/Dshubhambadola/Fortclaw/blob/main/scripts/make_appcast.sh)) and update `appcast.xml`.
...
- **npm pack/publish hangs or produces huge tarball**: the macOS app bundle in `dist/Fortclaw.app` (and release zips) get swept into the package. Fix by whitelisting publish contents via `package.json` `files` (include dist subdirs, docs, skills; exclude app bundles). Confirm with `npm pack --dry-run` that `dist/Fortclaw.app` is not listed.
...
- [ ] Attach artifacts: `npm pack` tarball (optional), `Fortclaw-X.Y.Z.zip`, and `Fortclaw-X.Y.Z.dSYM.zip` (if generated).
