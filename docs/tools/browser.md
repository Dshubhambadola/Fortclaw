...
Fortclaw can run a **dedicated Chrome/Brave/Edge/Chromium profile** that the agent controls.
...
- `chrome`: extension relay to your **system browser** (requires the Fortclaw
...
Fortclaw uses it automatically. Set `browser.executablePath` to override
auto-detection:
...
- **Remote CDP:** set `browser.profiles.<name>.cdpUrl` (or `browser.cdpUrl`) to
  attach to a remote Chromium-based browser. In this case, Fortclaw will not launch a local browser.

Remote CDP URLs can include auth:
- Query tokens (e.g., `https://provider.example?token=<token>`)
- HTTP Basic auth (e.g., `https://user:pass@provider.example`)

Fortclaw preserves the auth when calling `/json/*` endpoints and when connecting
to the CDP WebSocket. Prefer environment variables or secrets managers for
tokens instead of committing them to config files.

## Node browser proxy (zero-config default)

If you run a **node host** on the machine that has your browser, Fortclaw can
...
CDP endpoints over HTTPS. You can point a Fortclaw browser profile at a
...
Fortclaw supports multiple named profiles (routing configs). Profiles can be:
...
Fortclaw can also drive **your existing Chrome tabs** (no separate “clawd” Chrome instance) via a local CDP relay + a Chrome extension.
...
- You click the **Fortclaw Browser Relay** extension icon on a tab to attach (it does not auto-attach).
...
When launching locally, Fortclaw picks the first available:
1. Chrome
2. Brave
3. Edge
4. Chromium
...
Fortclaw supports two “snapshot” styles:
