...
import type { FortclawConfig } from "../src/config/config.js";
...
const createStubPlugin = (params: {
  ...
  config: {
    listAccountIds: (cfg: FortclawConfig) => {
    ...
    resolveAccount: (cfg: FortclawConfig, accountId: string) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
      const entry = channels?.[params.id];
      if (!entry || typeof entry !== "object") return {};
      const accounts = (entry as { accounts?: Record<string, unknown> }).accounts;
    ...
    isConfigured: async (_account, cfg: FortclawConfig) => {
      const channels = cfg.channels as Record<string, unknown> | undefined;
