...
export function listGoogleChatAccountIds(cfg: FortclawConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultGoogleChatAccountId(cfg: FortclawConfig): string {
...
export function listEnabledGoogleChatAccounts(cfg: FortclawConfig): ResolvedGoogleChatAccount[] {
