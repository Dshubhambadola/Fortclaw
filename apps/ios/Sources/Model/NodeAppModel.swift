...
final class NodeAppModel {
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = FortclawCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = FortclawCanvasA2UIAction.compactJSON(userAction["context"])
        let sessionKey = self.mainSessionKey

        let messageContext = FortclawCanvasA2UIAction.AgentMessageContext(
            actionName: name,
            session: .init(key: sessionKey, surfaceId: surfaceId),
            component: .init(id: sourceComponentId, host: host, instanceId: instanceId),
            contextJSON: contextJSON)
        let message = FortclawCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = FortclawCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    func requestLocationPermissions(mode: FortclawLocationMode) async -> Bool {
    ...
    private func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        do {
            ...
            case FortclawCameraCommand.list.rawValue,
                 FortclawCameraCommand.snap.rawValue,
                 FortclawCameraCommand.clip.rawValue:
                return try await self.handleCameraInvoke(req)
            case FortclawScreenCommand.record.rawValue:
                return try await self.handleScreenRecordInvoke(req)
            default:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: FortclawNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: FortclawNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case FortclawCanvasA2UICommand.reset.rawValue:
            ...
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case FortclawCanvasA2UICommand.push.rawValue, FortclawCanvasA2UICommand.pushJSONL.rawValue:
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(FortclawScreenRecordParams.self, from: req.paramsJSON)) ??
            FortclawScreenRecordParams()
...
private extension NodeAppModel {
    func locationMode() -> FortclawLocationMode {
