...
struct MacGatewayChatTransport: FortclawChatTransport, Sendable {
    func requestHistory(sessionKey: String) async throws -> FortclawChatHistoryPayload {
    ...
    func listSessions(limit: Int?) async throws -> FortclawChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(FortclawChatSessionsListResponse.self, from: data)
    }
    ...
    func sendMessage(
        ...
        attachments: [FortclawChatAttachmentPayload]) async throws -> FortclawChatSendResponse
    {
    ...
    func events() -> AsyncStream<FortclawChatTransportEvent> {
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> FortclawChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                FortclawGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    FortclawAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<FortclawChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any FortclawChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = FortclawChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: FortclawChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<FortclawChatView>) -> NSViewController
    {
