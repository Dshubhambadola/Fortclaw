...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: FortclawOAuthStore.AnthropicOAuthStatus = FortclawOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: FortclawOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = FortclawOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = FortclawOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try FortclawOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. Fortclaw can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: FortclawOAuthStore.AnthropicOAuthStatus,
