...
enum SessionMenuPreviewLoader {
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> FortclawSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: FortclawSessionPreviewEntry,
    ...
    private static func cache(payload: FortclawSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: FortclawChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [FortclawKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [FortclawKit.AnyCodable]) -> [FortclawChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(FortclawChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: FortclawChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: FortclawChatMessage) -> Bool {
    ...
    private static func toolNames(for message: FortclawChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: FortclawChatMessage) -> String? {
