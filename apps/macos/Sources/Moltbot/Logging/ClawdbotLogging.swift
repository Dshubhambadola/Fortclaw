...
enum FortclawLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = FortclawOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = FortclawFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        FortclawLogging.bootstrapIfNeeded()
        let label = FortclawLogging.makeLabel(subsystem: subsystem, category: category)
...
struct FortclawOSLogHandler: LogHandler {
...
struct FortclawFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = FortclawLogging.parseLabel(self.label)
